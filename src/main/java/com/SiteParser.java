...
public class SiteParser {
    ...
    public List<String> getNew(String url, User user) throws IOException {
        List<String> result = new ArrayList<>();
        List<ViewedHistory> viewedHistory = new ArrayList<>();

        String tmp = "";
        if (url.contains("&start")) {
            tmp = url.substring(0, url.indexOf("&start"));
            tmp = tmp + "&start=";
        } else {
            tmp = url + "&start=";
        }
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            String filterUrl = tmp.replace("m.rst.ua", "rst.ua") + i;
            ...
            Document doc = Jsoup.connect(filterUrl)
                    ...
                    .get();
            Elements elements;
            if (filterUrl.contains("m.rst.ua")) {
                elements = doc.select("#rst-mobile-oldcars-results > div.rst-uix-b-items");
            } else {
                elements = doc.select("#rst-page-" + i);
            }
            if (elements.isEmpty()) {
                LOGGER.log(Level.WARNING, "Url: " + filterUrl + " User: " + user.getId());
